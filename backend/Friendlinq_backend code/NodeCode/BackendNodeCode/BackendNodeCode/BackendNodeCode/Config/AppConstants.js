...
/**
 * SECURITY NOTE:
 * Do NOT hardcode secrets (GitHub will block pushes and it’s unsafe).
 * These must be provided via environment variables.
 */
let FCM_KEY = process.env.FCM_KEY || "";
//let FCM_KET = process.env.FCM_KET || "";  // (keeping your commented line, but safe)
...
// Twilio secrets moved to environment variables
// (keeping the old commented lines as comments is fine, but they contained secrets, so remove them)
let TWILIO_ACCOUNT_SID = process.env.TWILIO_ACCOUNT_SID || "";
let TWILIO_AUTH_TOKEN = process.env.TWILIO_AUTH_TOKEN || "";
let TWILIO_PHONE_NO = process.env.TWILIO_PHONE_NO || "";
