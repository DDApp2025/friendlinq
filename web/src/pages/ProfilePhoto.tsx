...
import { uploadProfilePic } from '../api/profile'
...
export default function ProfilePhoto() {
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (import.meta.env.DEV) {
      try {
        ...
        updateCurrentUserProfile({ profilePhoto: dataUrl })
  ...
  return (
    <ProfileEditLayout title="Profile Photo">
      {error && <div className="profile-error" role="alert">{error}</div>}
      <div style={{ padding: '20px', textAlign: 'center' }}>
        <p style={{ color: '#555', marginBottom: '20px' }}>Choose a new profile photo.</p>
        <input
          ref={fileInputRef}
          type="file"
          accept="image/*"
